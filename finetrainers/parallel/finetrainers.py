...
class FinetrainersParallelState(BaseParallelState):
    def __init__(
        ...
        if dp_shards == -1:
            self.dp_shards = world_size // (pp_degree * dp_degree * cp_degree * tp_degree)

        if self.dp_shards * pp_degree * dp_degree * cp_degree * tp_degree != world_size:
            raise ValueError(
    ...
    def data_parallel_enabled(self):
        return self.dp_degree > 1 or self.dp_shards > 1
    ...
    def data_sharding_enabled(self):
        return self.dp_shards > 1

    @property
    def context_parallel_enabled(self):
        return self.cp_degree > 1
